...
asyncTest("AJAX Tabs", 30, function (){
	...
	frameEl.load(p.ajax.attachDocumentListeners.scope(p.ajax));

	testUtils.setPath(frameEl, "/public_html/jqueryui/index.html", ok);
	p.testWaitForEvent("load", frameEl);
	p.wait(1911);
	p.testFireEvent("mousedown", "html>body>ul>li:eq(3)>a");
	p.wait(47);
	p.testFireEvent("click", "html>body>ul>li:eq(3)>a");
	p.testWaitForEvent("load", frameEl);
	p.wait(1621);
	...
	p.testFireEvent("mouseout", selector);
	p.wait(50);
	p.completeAjaxRequest({"responseType":"ajaxError","status":404,"statusText":"error","responseText":"Cannot GET /public_html/jqueryui/ajax/content4-broken.php","ajaxRequestId":16});
	p.wait(200);
	p.execCb(function (){
		ok(testUtils.$(frameEl, '#ui-tabs-4 p').length==0 && testUtils.$(frameEl, '#ui-tabs-1').html().length>0, "error notice is displayed");
	});
